...
public abstract class RequestHandler {
    ...
    public ByteBuffer render(final RequestData data) {
        ...
        // --- 新增：只處理 GET，攔截 HEAD 請求 ----------
        String method = data.exchange.getRequestMethod();
        if ("HEAD".equalsIgnoreCase(method)) {
            // 只回 HTTP 200 header，不要 body
            try {
                data.exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK, -1);
            } catch (IOException e) {
                logger.warning("Failed to handle HEAD request: " + e.getMessage());
            } finally {
                data.exchange.close();
            }
        ...
        // --- 新增：攔截 favicon.ico，回 404 或空白 ----------
        URI uri = data.exchange.getRequestURI();
        if (uri.getPath().endsWith("/favicon.ico")) {
            try {
                data.exchange.sendResponseHeaders(HttpURLConnection.HTTP_NOT_FOUND, -1);
            } catch (IOException e) {
                logger.warning("Failed to handle favicon request: " + e.getMessage());
            } finally {
                data.exchange.close();
            }
            return ByteBuffer.allocate(0);
