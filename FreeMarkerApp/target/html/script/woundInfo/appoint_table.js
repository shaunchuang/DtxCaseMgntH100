...
function generatePager(divclass, ajaxUrl, receiveData){
	var pages = 3
	var searching = false;
	var lengthChange = false;
	var info = false;
	var readData = receiveData == null ? null : testData;
	...
	$(divclass).dataTable({
        ...
        columns: [
            ...
            { data: null,
            	render: function ( data, type, row, meta ) {           		
            		return "<input type='checkbox' class='selectChk'>";
            	}
            },
            { data: "date" },
            { data: "name" },
        ...
        initComplete: function(){
        	...
        	$(".selectChk").change(function() {
        		var chkLeng = $('.patient-table tbody').find("input[type='checkbox']:checked").length;
        		if(chkLeng > 0 && $(".chk-enabled").hasClass('hidden')){
        			$('.chk-enabled').removeClass("hidden");
        		}
        		else if(chkLeng <= 0){
        			$('.chk-enabled').addClass("hidden");
        		}
        	});
        	
        	$(".dataTables_filter").prepend("<button class='chk-enabled hidden' onclick='save()'>儲存</button> <button class='chk-enabled hidden' onclick='unSelect()'>取消</button> <button class='all-chk' onclick='selectAll()'>全選</button>");
        },
        language: {
            search:         "搜尋:&nbsp;&nbsp;",
