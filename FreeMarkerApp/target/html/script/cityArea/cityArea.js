	...
	mlog.cityArea = {};
	$.extend(mlog.cityArea, {
		preprocess: function (str){
			if(str) str = str.replace("臺","台");
			return str;
		},
		
		selectCity: function (city){
			city = this.preprocess(city);
			$("#dynTown option").remove();
			$("#dynTown").append("<option value=''>請選擇</option>");
			if(oCity[city]!=null){
				for (var i=0; i< oCity[city].length;i++ ){
					$("#dynTown").append("<option value='"+oCity[city][i][0]+"'>"+oCity[city][i][0]+"</option>");
				}
			}
			$("#city").val(city);
		},
		...
		setZipCode: function (code){
			if(code){
				var found = false;
				for (var city in oCity ){
					for (var i=0; i< oCity[city].length;i++ ){
						if(oCity[city][i][1]==code){
							this.selectCity(city);
							this.selectArea(oCity[city][i][0]);
							found = true;
							break;
						}
					}
		...
		getZipCode: function (){
			if($("#city").val()){
				...
				if($("#dynTown").val()){
					...
					for (var i=0; i< oCity[city].length;i++ ){
						if(oCity[city][i][0]==area){
							return [oCity[city][i][1]];
						}
					}
				}else{
					if(oCity[city].length>0){
						var zips = [];
						for (var i=0; i< oCity[city].length;i++ ){
							zips.push(oCity[city][i][1]);
						}
						return zips;
					}
				}
		...
		initCityArea: function (zips){
			this.checkIndexOf();
			if(zips){
				zips.sort();
				for (var city in oCity ){
					for (var i=0; i< oCity[city].length; i++){
						if(zips.indexOf(oCity[city][i][1])==-1){
							oCity[city].splice(i, 1);
							i--;
						}
					}
					if(oCity[city].length==0){
			...
			$("#city").change(function() {
				mlog.cityArea.selectCity($(this).val());
			});
		},
