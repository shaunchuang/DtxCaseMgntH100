		...
		<div class="custom-blk">
			...
			<div class="col-md-12 sub-bar">
				<div class="col-md-6 item-title">
					<div class="item-name"><@spring.message "diagnosis.list.label.title"/></div>
				</div>				           
			</div>
			<!-- 各功能模組內容放置區 -->
			...
			<div class="main-content">
				<div class="col-md-12 pd-h-5 justify-content-between">
	            	<div class="input-group search-group">																		
						<span class="input-group-addon"><@spring.message "diagnosis.list.label.ptno"/></span>						
						<input type="search" class="form-control ptNo" aria-describedby="basic-addon1" >
						<span class="input-group-addon"><@spring.message "diagnosis.list.label.idno"/></span>						
						<input type="search" class="form-control idNo" aria-describedby="basic-addon1" >
						<span class="input-group-addon"><@spring.message "diagnosis.list.label.name"/></span>						
						<input type="search" class="form-control ptName" aria-describedby="basic-addon1" >	
						<span class="input-group-addon"><@spring.message "diagnosis.list.label.dateRange"/></span>
						<input type="search" class="form-control startDatePicker" aria-describedby="basic-addon1" placeholder="<@spring.message "diagnosis.list.placeholder.startDate"/>">
						<span class="input-group-addon">~</span>
						<input type="search" class="form-control endDatePicker" aria-describedby="basic-addon1" placeholder="<@spring.message "diagnosis.list.placeholder.endDate"/>">
						...
						<table class="table table-striped table-bordered bootstrap-datatable table-customed main-table">
							<thead>
						      <tr class="tablesorter-headerRow">
						      	<th width="15%"><@spring.message "diagnosis.list.column.ptno"/></th>
						      	<th width="30%"><@spring.message "diagnosis.list.column.idno"/></th>
						      	<th width="20%"><@spring.message "diagnosis.list.column.name"/></th>					      	
						      	<th width="20%"><@spring.message "diagnosis.list.column.evalDate"/></th>
						        <th width="15%"><@spring.message "diagnosis.list.column.funcItem"/></th>
						      </tr>
						    </thead>
							<tbody id="list_cnt">
...
var columnObj = [
    ...
    { data: null,
    	render: function ( data, type, row, meta ) {
    		return '<button class="func-btn mg-right-5" onclick="exportCertificate(\'' + row.keyno + '\',\'' + row.evalDate + '\')"><i class="fa fa-print"></i> <@spring.message "diagnosis.list.button.print"/></button>';
    	}
    }
];
...
//顯示個案特定看診日之檢測報告
function exportCertificate(targetId, evalDate){
	window.open("${base}/${__lang}/division/web/report/diagnosis/print/" + targetId + "/evalDate/" + evalDate);
}
</script>
