...
function initData() {
	addTableHead();
	$.get("cgFormHeadController?getColumnList&id=" + $("#id").val(),
			getDataHanlder);
	$.get("cgFormIndexController?getIndexList&id=" + $("#id").val(),
			getDataHanlderIndex);
...
function deleteUnUsedFiled(){
	if(toDelete && toDelete.length>0){
		for(index in toDelete){
			...
			$.post("cgFormHeadController?delField&id="+ toDelete[index]);
		}
	}
	return true;
}
...
function delColumnBtnClick() {
	...
	$("#tab_div_database").find("input[name='ck']:checked").parent().parent("tr").each(function(index, ele){
		//$.post("cgFormHeadController?delField&id="+ $("#tab_div_database").find("input[name='ck']:checked").val());
