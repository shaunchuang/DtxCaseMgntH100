	var oCity = {
		...
		"宜蘭縣": [["宜蘭市","260"],["頭城鎮","261"],["礁溪鄉","262"],["壯圍鄉","263"],["員山鄉","264"],["羅東鎮","265"],["三星鄉","266"],["大同鄉","267"],["五結鄉","268"],["冬山鄉","269"],["蘇澳鎮","270"],["南澳鄉","272"],["釣魚台","290"]],
	...
	function setZipCode(code){
		if(code){
			...
			for (var key in oCity ){
				for (var i=0; i< oCity[key].length;i++ ){
					if(oCity[key][i][1]==code){
						$("#city").val(key);
						setDynTown(key);
						$("#dynTown").val(oCity[key][i][0]);
						found = true;
						break;
					}
				}
	...
	function getZipCode(){
		if($("#city").val()){
			var key = $("#city").val();
			if($("#dynTown").val()){
				var area = $("#dynTown").val();
				for (var i=0; i< oCity[key].length;i++ ){
					if(oCity[key][i][0]==area){
						return [oCity[key][i][1]];
					}
				}
			}else{
				if(oCity[key].length>0){
					var zips = [];
					for (var i=0; i< oCity[key].length;i++ ){
						zips.push(oCity[key][i][1]);
					}
					return zips;
				}
			}
	...
	function setDynTown(key, idCity, idDynTown) {
		if (typeof(idCity)==='undefined') idCity = "#city";
		if (typeof(idDynTown)==='undefined') idDynTown = "#dynTown";
		
		$(idDynTown + " option").remove();
		$(idDynTown).append("<option value=''>請選擇</option>");
		if(oCity[key]!=null){
			for (var i=0; i< oCity[key].length;i++ ){
				$(idDynTown).append("<option value='"+oCity[key][i][0]+"'>"+oCity[key][i][0]+"</option>");
			}
		}
	}
	
	function initCityArea(zips, idCity, idDynTown){
		if (typeof(idCity)==='undefined') idCity = "city";
		if (typeof(idDynTown)==='undefined') idDynTown = "dynTown";
		...
		if(zips){
			...
			for (var key in oCity ){
				for (var i=0; i< oCity[key].length; i++){
					if(zips.indexOf(oCity[key][i][1])==-1){
						oCity[key].splice(i, 1);
						i--;
					}
				}
				if(oCity[key].length==0){
		...
		$(idCity).append("<option value=''>請選擇</option>");
		
		for (var key in oCity ){
			$(idCity).append("<option value='"+key+"'>"+key+"</option>");
		}
		
		setDynTown("", idCity, idDynTown);
		$(idCity).change(function() {
			setDynTown($(this).val(), idCity, idDynTown);
		});
	}
	/*
	function setCityArea(address){
		var pos = -1;
		var start = -1;
		address = address.replace("臺","台");
		for (var key in oCity ){
			pos = address.indexOf(key); 
			if(pos!=-1){
				$("#city").val(key);
				setDynTown(key);
				start = pos + key.length;
				for (var i=0; i< oCity[key].length;i++ ){
					if(address.indexOf(oCity[key][i][0],start)==start){
						$("#dynTown").val(oCity[key][i][0]);
						return start + oCity[key][i][0].length;
					}
				}
				return start;
			}
