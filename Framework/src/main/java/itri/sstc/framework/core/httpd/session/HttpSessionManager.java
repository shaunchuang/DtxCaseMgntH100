...
public class HttpSessionManager extends TimerTask {
    ...
    public HttpSession getSession(HttpExchange exchange, boolean auto) {
        ...
        /**
     * 取得 HttpSession (SessionId 紀錄在 Cookie 中)
     * @param exchange
     * @param auto - 如果 true 而找不到 session，則新建一個
     */
    public HttpSession getSession(HttpExchange exchange, boolean auto) {
        HttpSession session = null;
        Map<String,String> cookies = CookieParser.parse(exchange.getRequestHeaders());
        // 如果前端有帶 cookie，嘗試從 sessions 裡取
        if (cookies.containsKey(HttpSession.COOKIE_NAME)) {
            String sid = cookies.get(HttpSession.COOKIE_NAME);
            session = sessions.get(sid);
        }
        // 如果沒有找到，而且 auto=true，就新建
        if (session == null && auto) {
            session = createSession(exchange);

            // **重點：新建完一定要把 Set-Cookie 寫回去**
            Headers resp = exchange.getResponseHeaders();
            StringBuilder sb = new StringBuilder();
            sb.append(HttpSession.COOKIE_NAME)
              .append("=")
              .append(session.getSessionId())
              .append("; Path=/; HttpOnly; SameSite=None; Secure");
            // 如果你有設定 domain
            ...
            resp.add("Set-Cookie", sb.toString());
        }
        return session;
    }
